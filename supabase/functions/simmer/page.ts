...
<style>
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  .tabbar { position: fixed; left: 0; right: 0; bottom: 0; z-index: 40; display: flex; gap: 6px;
  ...
  .gwrap { padding: 14px 16px; max-width: 720px; margin: 0 auto; }
  .gactions { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; gap: 8px; }
  .gviews { display: flex; gap: 6px; }
  .gview { border: 1px solid var(--line); background: var(--card); color: var(--text); border-radius: 999px;
    padding: 7px 13px; font-size: 13px; font-weight: 600;
    transition: background-color .2s, color .2s, border-color .2s, transform .15s; }
  .gview:active { transform: scale(.94); }
  .gview.active { background: var(--blue); border-color: var(--blue); color: #fff; }
  .gaisle { font-family: var(--serif); font-size: 17px; font-weight: 700; margin: 18px 0 2px;
    padding-bottom: 6px; border-bottom: 2px solid var(--line); }
  .gaisle:first-child { margin-top: 4px; }
  .gaisle .n { color: var(--muted); font-size: 12.5px; font-weight: 600; font-family: -apple-system, BlinkMacSystemFont, sans-serif; margin-left: 6px; }
  .gclear { border: 1px solid var(--line); background: var(--card); color: var(--muted); border-radius: 999px;
    padding: 7px 13px; font-size: 13px; font-weight: 600; }
  .grow { display: flex; align-items: flex-start; gap: 12px; padding: 12px 4px; border-bottom: 1px solid var(--line); }
  .grow:last-child { border-bottom: none; }
  .gcheck { width: 22px; height: 22px; border-radius: 8px; border: 2px solid var(--muted); flex: 0 0 22px;
    margin-top: 2px; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 14px; }
  .grow.done .gcheck { background: var(--accent); border-color: var(--accent); }
  .gname { font-size: 15.5px; font-weight: 700; text-transform: capitalize; }
  .gtotal { color: var(--accent); font-size: 13px; font-weight: 800; margin-left: 8px; text-transform: none; }
  .grow.done .gtotal { color: var(--muted); }
  .grow.done .gname { text-decoration: line-through; color: var(--muted); }
  .gsub { color: var(--muted); font-size: 12.5px; margin-top: 2px; line-height: 1.45; }
  ...
  .cartbtn { border: 1px solid var(--line); background: var(--card); color: var(--accent); border-radius: 9px;
    width: 30px; height: 30px; font-size: 15px; font-weight: 700; flex: 0 0 30px; margin-top: -2px; }
  .cartbtn:disabled { color: var(--muted); }
  ...
  .convrow { display: flex; align-items: baseline; gap: 10px; padding: 11px 2px; border-bottom: 1px solid var(--line); }
  .convrow:last-of-type { border-bottom: none; }
  .conveq { color: var(--muted); font-size: 16px; }
  .convval { font-family: var(--serif); font-size: 21px; font-weight: 700; }
  .convapprox { color: var(--muted); font-size: 12px; font-style: italic; margin-left: auto; }
  ...
  .scalechip.active { background: var(--blue); border-color: var(--blue); color: #fff; }

  .cookbtn { border: none; background: var(--accent); color: #fff; border-radius: 999px;
    padding: 7px 14px; font-size: 12.5px; font-weight: 800; letter-spacing: .3px; }
  #cook { position: fixed; inset: 0; z-index: 70; background: var(--bg); display: none; flex-direction: column; }
  #cook.open { display: flex; animation: fadein .22s ease; }
  @keyframes fadein { from { opacity: 0; } }
  .cooktop { display: flex; align-items: center; justify-content: space-between; gap: 10px;
    padding: calc(12px + env(safe-area-inset-top)) 14px 6px; }
  .cookcount { font-size: 12.5px; font-weight: 800; letter-spacing: 1.6px; color: var(--muted);
    text-transform: uppercase; text-align: center; flex: 1; }
  .cookdots { display: flex; gap: 5px; justify-content: center; flex-wrap: wrap; padding: 4px 20px 0; }
  .cookdot { width: 7px; height: 7px; border-radius: 999px; background: var(--line);
    transition: background-color .25s, transform .25s; }
  .cookdot.on { background: var(--accent); }
  .cookdot.now { transform: scale(1.35); }
  .cookstep { flex: 1; display: flex; align-items: center; justify-content: center;
    padding: 12px 28px 24px; overflow-y: auto; cursor: pointer; }
  .cooktxt { font-family: var(--serif); font-size: 26px; line-height: 1.5; font-weight: 600;
    max-width: 640px; animation: stepin .28s cubic-bezier(.22,.9,.3,1) both; }
  @keyframes stepin { from { opacity: 0; transform: translateY(10px); } }
  .cookdone { text-align: center; }
  .cookdone .big { font-size: 64px; margin-bottom: 10px; animation: bounceonce .5s cubic-bezier(.22,.9,.3,1); }
  @keyframes bounceonce { 0% { transform: scale(.6); opacity: 0; } 60% { transform: scale(1.12); } 100% { transform: scale(1); opacity: 1; } }
  .cookbottom { display: flex; gap: 10px; padding: 0 16px calc(16px + env(safe-area-inset-bottom)); }
  .cooknavbtn { flex: 1; border: 1px solid var(--line); background: var(--card); color: var(--text);
...
<div id="groceryview" class="gwrap" style="display:none">
  ...
  <div class="gempty" id="gempty" style="display:none">
    <div class="big">🛒</div>
    <p>Your grocery list is empty.<br>Open a recipe and tap <b>+</b> next to an ingredient,<br>or <b>Add all to list</b>.</p>
  </div>
</div>
...
(function () {
  ...
  $("cookpeek").onclick = function () {
    ...
    cook.ings.forEach(function (it) {
      var row = el("div", "ing");
      var lb = el("label", null, it); lb.style.flex = "1";
      row.appendChild(lb);
      box.appendChild(row);
    });
