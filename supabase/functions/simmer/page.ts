...
<style>
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  /* a scan is the source of truth — show the whole card, never crop it */
  .webimgwrap.photowrap { max-height: 64vh; background: var(--sand); }
  .webimgwrap.photowrap img { max-height: 64vh; object-fit: contain; }
  ...
  .secondary { width: 100%; border: 1px solid var(--line-2); border-radius: 16px; padding: 15px;
  ...
  .secondary:active { transform: scale(.985); border-color: var(--ember); color: var(--ember-ink); }
  .secondary:disabled { opacity: .55; }
  .orline { display: flex; align-items: center; gap: 12px; margin: 20px 0 15px; color: var(--muted);
    font-size: 10px; font-weight: 700; letter-spacing: .18em; text-transform: uppercase; }
  .orline::before, .orline::after { content: ""; flex: 1; height: 1px; background: var(--line-2); }
  .sheetbody p.scanhint { margin: 11px 0 0; text-align: center; font-size: 12px; }
...
<div class="sheet" id="addsheet">
  <div class="sheetbody">
    ...
    <button class="secondary" id="scanbtn">📷 Scan a photo of a recipe</button>
    <p class="scanhint">A recipe card, a cookbook page, or your own handwriting.</p>
...
(function () {
  ...
  function openDetail(r) {
    ...
    if (r.platform === "web" || r.platform === "photo") {
      // no video to embed — show the cached page image, or the scan itself
      if (r.thumb_url) {
        var ww = el("div", "webimgwrap" + (r.platform === "photo" ? " photowrap" : ""));
    ...
    link.textContent = (r.platform === "tiktok" ? "Open in TikTok ↗"
      ...
      : r.platform === "photo" ? "View the photo ↗"
      : "Open in Instagram ↗");
    mr.appendChild(link);
    c.appendChild(mr);
    ...
    if (subs.length) {
      ...
      nb.appendChild(el("div", "norecipe",
        r.platform === "photo"
          ? "Couldn't read a recipe from this photo — tap ↻ up top to try again, or scan a straighter, better-lit shot."
        : r.platform === "web"
  ...
  function renderCook() {
    ...
    // On the finish screen the nav row would be a single orphaned arrow, so drop it —
    // "Finish cooking" is the only thing left to do, and a swipe still steps back.
    document.querySelector(".cookbottom").style.display = done ? "none" : "";
  }
  ...
  var SCAN_LABEL = "📷 Scan a photo of a recipe";

  // Phone photos are 4-12MB; Gemini caps at 4MB and the edge function has to carry it
  // as base64, so shrink to something a recipe card is still legible at.
  function downscale(file, maxDim, quality, cb) {
    ...
    reader.onload = function () {
      ...
      img.onload = function () {
        ...
        try { cb(cv.toDataURL("image/jpeg", quality)); } catch (e) { cb(null); }
      };
  ...
  function scanDone(msg) {
    var b = $("scanbtn");
    b.disabled = false; b.textContent = SCAN_LABEL;
    if (msg) toast(msg);
  }

  $("scanbtn").onclick = function () { $("scaninput").click(); };
  $("scaninput").onchange = function () {
    var input = $("scaninput");
    var f = input.files && input.files[0];
    input.value = "";  // so picking the same photo twice still fires a change
    if (!f) return;
    var b = $("scanbtn");
    b.disabled = true; b.textContent = "Opening the photo…";
    downscale(f, 1600, 0.82, function (dataUrl) {
      if (!dataUrl) { scanDone("Couldn't open that photo"); return; }
      b.textContent = "Reading the recipe… (10–20s)";
      api("ingest-photo", { method: "POST", body: JSON.stringify({ image: dataUrl }) }).then(function (res) {
        scanDone((res && res.message) || "Done");
        if (res && (res.status === "saved" || res.status === "exists")) {
          $("addsheet").classList.remove("open");
          // open what was read straight away, so a bad scan is obvious immediately
          load().then(function () {
            for (var i = 0; i < state.recipes.length; i++) {
              if (state.recipes[i].id === res.id) { openDetail(state.recipes[i]); return; }
            }
          });
        }
      }).catch(function () { scanDone("Network error"); });
    });
  };
